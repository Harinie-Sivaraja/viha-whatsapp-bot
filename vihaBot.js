...
app.listen(PORT, () => {
    ...
    console.log(`Visit your Railway app URL to see QR code`);
});
...
const sendUnder50Images = async (chatId) => {
    try {
        ...
        if (imageFiles.length === 0) {
            ...
            // Send final message if no images found
...
// Function to generate short customer summary
const generateShortSummary = (userStateData) => {
    const budgetOptions = {
        '1': 'Budget Under ₹50',
        '2': 'Budget ₹51 - ₹100',
        '3': 'Budget ₹101 - ₹150',
        '4': 'Budget ₹151 - ₹200',
        '5': 'Budget More than ₹200'
    };

    return `*Your Requirements:* ${budgetOptions[userStateData.budget] || 'Budget not specified'}`;
};
...
client.on('message', async message => {
    try {
        ...
        if (userState[chatId] && userState[chatId].step === 'completed') {
            ...
            // Don't respond to avoid confusion - human agent should handle follow-up questions
            return;
        }
